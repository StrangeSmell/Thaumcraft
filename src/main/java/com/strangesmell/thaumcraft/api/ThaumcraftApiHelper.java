...
import com.strangesmell.thaumcraft.api.aspects.IEssentiaTransport;
...
/*    public static boolean isResearchComplete(String username, String researchkey) {
    ...
    {   //modify 5.25 22:22 by strange_smell
        if (recipeItem == null || slotItem == null) return false;
        if (recipeItem.getTag()!=null && slotItem.getTag()==null ) return false;
        if (recipeItem.getTag()==null ) return true;
        //???
        Iterator iterator = recipeItem.getTag().getAllKeys().iterator();
        while (iterator.hasNext())
        {
            String s = (String)iterator.next();
            if (slotItem.getTag().contains(s)) {
                if (!slotItem.getTag().get(s).toString().equals(
                        recipeItem.getTag().get(s).toString())) {
                    return false;
                }
            } else {
                return false;
    ...
    {   //modify 5.25 22:22 by strange_smell
        ...
        //???
        //return (target.getItem() == input.getItem() && ((target.getDamageValue() == OreDictionary.WILDCARD_VALUE && !strict) || target.getDamageValue() == input.getDamageValue()));
    ...
    public static BlockEntity getConnectableTile(LevelAccessor world, int x, int y, int z, Direction face) {
        //modify 5.25 22:22 by strange_smell
        //???
        BlockEntity te = world.getBlockEntity(new BlockPos(x+face.getStepX(), y+face.getStepY(), z+face.getStepZ()));
        if (te instanceof IEssentiaTransport && ((IEssentiaTransport)te).isConnectable(face.getOpposite()))
...
/*
    ...
    *//**
    ...
    public static MovingObjectPosition rayTraceIgnoringSource(Level world, Vec3 v1, Vec3 v2,
